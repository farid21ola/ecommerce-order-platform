...
func (r *ConsumerRunner) Run(ctx context.Context) {
	for {
		...
		if err != nil {
			...
			log.Printf("readmodel kafka consumer %s fetch failed: %v", r.topic, err)
			time.Sleep(time.Second)
			continue
		}

		if err := r.handler.HandleEvent(ctx, message.Event); err != nil {
			log.Printf("readmodel kafka consumer %s handle %s failed: %v", r.topic, message.Event.EventType, err)
			continue
		}

		if err := r.consumer.Commit(ctx, message); err != nil {
			log.Printf("readmodel kafka consumer %s commit failed: %v", r.topic, err)
		}
	}
}
